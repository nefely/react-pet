...
export default function AddAnimeModal({ show, onClose, onSaved }) {
  ...
  const previewOk = useMemo(() => {
    try { if (!form.image) return false;
        ...
        <div className="modal-content">
          <form onSubmit={submit}>
            <div className="modal-header">
              <h5 className="modal-title">Додати аніме</h5>
            ...
            <div className="modal-footer">
              <button type="button" className="btn btn-outline-secondary" onClick={onClose}>Скасувати</button>
              <button className="btn btn-success" disabled={saving}>{saving ? "Зберігаю…" : "Додати"}</button>
            </div>
          </form>
        </div>
      </div>
