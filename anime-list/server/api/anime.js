...
router.post("/", authRequired ,async (req, res) => {
    try {
        ...
        const [result] = await pool.execute("INSERT INTO anime_list (image, name, seasons, rating) VALUES (?, ?, ?, ?)", [image.trim(), name.trim(), seasons.trim(), Number(rating)]);
...
router.put("/:id", authRequired ,  async (req, res) => {
    try {
        ...
        const [result] = await pool.execute("UPDATE anime_list SET name=?, image=?, seasons=?, rating=? WHERE id=?", [name.trim(), image.trim(), seasons.trim(), Number(rating), Number(id)]);
...
router.delete("/:id", authRequired , async (req, res) => {
  try {
    ...
    console.error("DB delete error:", e.code, e.message);
    res.status(500).json({ code: e.code, message: e.message });
  }
});
